...
- **2026-09-26 复核（宿主进到 `0.1.7-rc.2`，实跑在桌面版里）**：本仓编译所依 bump 到 `0.1.7-rc.2`。两代之间 `@deepseek-ai/dsh-tools` 的类型面增量**只有一处**——`PreToolDecision` 的 `ask` 分支多一个可选 `displayReason`（本地取两版 tarball 现 diff `lib/types/index.d.ts` 得，7 行差异、无删改）；本插件唯一消费的宿主工具面符号是 `defineTool`，一字未动。宿主带的共享包仍是 `@deepseek-ai/cordis 4.0.4` / `@deepseek-ai/schemastery 3.18.4`，与本仓 peer 正对。桌面版的运行时面另记一条：它自带 node / pnpm 与 `dsh` CLI（`resources/app.asar/dsh/…`），`hostProtocolVersion` 在 `dsh/desktop-runtime.json` 里可读。
