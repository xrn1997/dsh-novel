...
现查：对面 `.kt` 必须是**带目录的相对路径**且在该仓存在、任何引用不许带行号、`.superpowers/`
只许是登记过的工具输出目录。参考仓不在场即红（`DSH_LEGADO_REF` 指路，`=off` 是唯一显式退路）。
...
| `不适用：列表规则 + 前缀` | 列表规则的 `+` 前缀（历史 all-in-one 标记） | 对面**只剥不做事**，剥掉即等价于不识别；本仓按未知段处理会在解析期炸。真机库 158 源零出现——出现该形态的源再按那一源放宽。 |
