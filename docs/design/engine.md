...
| `js-sandbox.ts` 的 no-op 名单：`需要安卓宿主环境` | `java.webView`/crypto/`android.*`/`org.*` → 报「需要安卓宿主环境」 | 不静默 no-op；纯 UI 副作用（toast/copyText/startBrowser/open）则明确 no-op。~~`getString(rule, isUrl=true)` 曾列在本表（「在 v1 不支持：不在桥内做 fetch」）~~ **该守门建在误读上，已随 2026-09-23 那批删除**：对面的 isUrl 从不抓取，只做绝对化（见「legado 取值语义订正」第 5 条）——本仓仍然不在桥内 fetch，但不再有合法调用被判成未实现 |
...
即 `source.get/put` 那张按源隔离的**键值表**（对面 `v_<source>_<key>`）。**`getVariable` 不是它**——
对面那是另一处单串槽（`sourceVariable_<source>`），本仓同批拆成 `SourceSession.sourceString`，
...
5. **`java.getString` 的三重载分派**（`js-protocol.ts` 的 `parseGetStringArgs`，同批续）。对面是两个重载加一个四参缺省：`getString(rule, unescape: Boolean)`（第二参布尔 = **要不要再做一次 HTML 反转义**，缺省 true）与 `getString(rule, mContent, isUrl)`（第二参是基内容、第三参才是 isUrl，而 isUrl **只做绝对化、从不抓取**：空白回退 base，否则 `NetworkUtils.getAbsoluteURL(redirectUrl, str)`；列表出口逐项绝对化 + 去重）。本仓只有一个 `(rule, isUrl?)` 签名 ⇒ `true` 被判成未实现直接抛，`false` 不抛但把「不要再解一次实体」静默反做（页面 `<p>&amp;lt;</p>` 上对面 `false` 要 `&lt;`）。现按实参类型分派，绝对化收在新的纯函数口 `js-utils.absolutizeUrl`（对面 getAbsoluteURL 五条口径逐条搬：base 空返 trim 原串、base 先剥 `,` 后的请求选项、绝对地址与 data URI 原样、`javascript` 开头给空串、解析失败回退），解码收在 `js-utils.unescapeHtml4`。
   **两处如实标注的近似**：① 桥内 base 用 `EvalContext.baseUrl`，对面用 `redirectUrl`（重定向落地地址）——本仓落地地址只在 HTTP 面（`search-face.landedUrl`）留存，脚本路径拿不到，故被重定向过的站点上相对段可能落在未跟随的 base 下；② `unescapeHtml4` 是**白名单近似**（数字引用 + HTML4 常用命名集），对面 commons-text 是全表，认不出的一律原样留——差集是「它能解、我们留原文」，不是解成别的值。钉子按「零 fetch 通道」自证：桥的 deps 里没有 fetch，实现若去抓站点会当场抛「该源未提供网络能力」。
   **删掉一条守门**：`'getString 的 isUrl=true 在 v1 不支持'` 这个抛错连同它在「宁炸不猜」表和审计锚点（`tests/content-audit-classify.ts` 的 HOST_GAP）里的登记一并撤下——守门本身建在误读上，留着会让下一个人以为对面会抓取。**它仍不在桥内 fetch**，只是不再把合法调用判成未实现。

6. **源级状态拆成三张表**（`js-sandbox.ts` 的 `SourceSession` + `js-protocol.ts` 的六个方法）。对面 `data/entities/BaseSource.kt` 写得很清楚：`setVariable/putVariable/getVariable` 是**一个字符串槽**（CacheManager 键 `sourceVariable_<source>`，`getVariable` 直返那串、未设返 `""`），`put(key,value)/get(key)` 是**另一个命名空间**（键 `v_<source>_<key>`，缺键返 `""`、`put` 返回写入值），`cache.*` 又是第三处（CacheManager 全局表，本仓按源隔离是在册裁决）。本仓此前把前两件事塞进同一张 Map：`sourceSetVariable` 先 `JSON.parse` 再 `clear()` 整表 ⇒ ① `getVariable()` 给出 `{"":"abc"}` 而不是 `abc`（存自定义域名的源直接拿到一层 JSON 壳）；② 写串槽把 `source.put` 写过的键全清空；③ cache 借同一张表加 `cache:` 前缀，`source.get("cache:x")` 能读到缓存项而对面读不到。现在三张表各自建档，缺键按对面返 `""`、`put` 回传值，`java.get` 的 source 层仍只读键值表。
   **没顺手做的**：对面这三处都经 CacheManager **落盘跨启动**，本仓全在进程内。落盘给数据根添一张源级状态表、连带「删源/换规则时清不清」的口径，与 `h-cache-ttl` 是同一条轴 ⇒ 单列矩阵行 `h-source-variable-persistence` 排队，不在形状修复里替这个决策拍板。

**对读出来、刻意没在这两批动的**（都已在矩阵登记为 open，附对面锚点）：
